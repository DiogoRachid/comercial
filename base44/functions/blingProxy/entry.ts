...
Deno.serve(async (req) => {
  ...
  if (action === 'listOrders') {
    ...
    const qs = new URLSearchParams({ pagina: String(pagina), limite: String(limite) });
    if (dataInicio) qs.set('dataInicio', dataInicio);
    const data = await blingRequest(accessToken, `/pedidos/vendas?${qs.toString()}`);
    return Response.json(data);
  }
  ...
  if (action === 'updateProduct') {
    const accessToken = await getValidAccessToken(base44);
    const { blingId, produto } = payload;
    const data = await blingRequest(accessToken, `/produtos/${blingId}`, {
      method: 'PUT',
  ...
  if (action === 'createOrder') {
    const accessToken = await getValidAccessToken(base44);
    const data = await blingRequest(accessToken, '/pedidos/vendas', {
      method: 'POST',
  ...
  if (action === 'updateOrderStatus') {
    const accessToken = await getValidAccessToken(base44);
    const { blingOrderId, situacaoId } = payload;
    const data = await blingRequest(accessToken, `/pedidos/vendas/${blingOrderId}/situacoes`, {
      method: 'PATCH',
      body: JSON.stringify({ situacao: { id: situacaoId } }),
    });
    return Response.json(data);
  }
  ...
  if (action === 'createContact') {
    const accessToken = await getValidAccessToken(base44);
    const data = await blingRequest(accessToken, '/contatos', {
      method: 'POST',
      body: JSON.stringify(payload.contato),
    });
    return Response.json(data);
  }
  ...
  if (action === 'updateStock') {
    const accessToken = await getValidAccessToken(base44);
    const data = await blingRequest(accessToken, '/estoques', {
      method: 'POST',
      body: JSON.stringify(payload.estoque),
    });
    return Response.json(data);
  }
  ...
  if (action === 'listStock') {
    ...
    const data = await blingRequest(accessToken, `/estoques?idsProdutos=${ids}`);
    return Response.json(data);
  }

  if (action === 'getNFe') {
    const accessToken = await getValidAccessToken(base44);
    const { blingOrderId } = payload;
    const data = await blingRequest(accessToken, `/nfe?idPedidoVenda=${blingOrderId}`);
    return Response.json(data);
  }

  if (action === 'listDeposits') {
    const accessToken = await getValidAccessToken(base44);
    const data = await blingRequest(accessToken, '/depositos');
    return Response.json(data);
  }

  if (action === 'listStores') {
    const accessToken = await getValidAccessToken(base44);
    const data = await blingRequest(accessToken, '/lojas');
    return Response.json(data);
  }
  ...
  if (action === 'listContacts') {
    ...
    const data = await blingRequest(accessToken, `/contatos?${qs.toString()}`);
    return Response.json(data);
  }
  ...
  if (action === 'listPurchaseOrders') {
    const accessToken = await getValidAccessToken(base44);
    const pagina = payload.pagina || 1;
    const data = await blingRequest(accessToken, `/pedidos/compras?pagina=${pagina}&limite=100`);
    return Response.json(data);
  }
